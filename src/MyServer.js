...
class MyServer {
    ...
    static async broadcastMessage(wss, message, roomID, userToExclude = null) {
        wss.clients.forEach((client) => {
            if (client.readyState === 1 && client.roomID === roomID) {
                if (
                    !(userToExclude !== null && client.user === userToExclude)
                ) {
                    client.send(message);
                }
            }
    ...
    start(port) {
        ...
        server.on("upgrade", (request, socket, head) => {
            ...
            const parsedParams = qs.parse(parsedUrl.query, {
                    ...
                    Parser.parse(data)
                        .then(async (req) => {
                            if (req.method === "sendMessage") {
                                await MyServer.broadcastMessage(
                                    wss,
                                    JSON.stringify({ message: req.message }),
                                    req.roomID,
                                    req.user
                                );
                                ws.send(
                                ...
                                const users = MyServer.getUsersFromRoom(
                                    wss,
                                    req.roomID
                                );
                                ws.send(
